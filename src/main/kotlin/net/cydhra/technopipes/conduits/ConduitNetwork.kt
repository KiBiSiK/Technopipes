...
object ConduitNetwork {
    ...
    fun tick(world: WorldServer) {
        if (dimensions.containsKey(world.provider.dimension)) {
            dimensions[world.provider.dimension]!!.tick(world)
        }
    }
    ...
    fun onChunkBeginWatching(event: ChunkWatchEvent.Watch) {
        ...
        if (!world.isRemote) {
            ...
            val networkChunk = dimensions[dim]!!.getChunkAt(chunk.pos)!!
    ...
    fun onWorldUnload(event: WorldEvent.Unload) {
        dimensions.remove(event.world.provider.dimension)
    }
    ...
    fun receiveNetworkChunk(dimension: Int, chunkPos: ChunkPos, data: NBTTagCompound) {
        if (!Minecraft.getMinecraft().isSingleplayer) {
            val dim = dimensions.getOrPut(dimension) { ConduitNetworkDimension(dimension) }

...
import net.cydhra.technopipes.conduits.parts.AttachmentPart
import net.cydhra.technopipes.conduits.parts.EdgePart
import net.cydhra.technopipes.conduits.parts.NodePart
import net.cydhra.technopipes.conduits.parts.Part
import net.cydhra.technopipes.conduits.transit.TransitChunkEdge
import net.cydhra.technopipes.conduits.transit.TransitCrossSectionEdge
import net.cydhra.technopipes.conduits.transit.TransitEdge
import net.cydhra.technopipes.conduits.transit.TransitSink
    ...
    private fun addCrossSection(pos: BlockPos, type: PipeType, face: EnumFacing) {
        ...
        if (this.chunkCrossSections[pos]!!.contains(type)) {
            ...
            this.edges[pos]!![type]!!.forEach { edgeFace ->
                this.crossSectionTransitEdges[pos]!!.add(
                    TransitCrossSectionEdge(
                        transitEdgeCounter++,
                        type,
                        edgeFace,
                        pos
    ...
    override fun deserializeNBT(nbt: NBTTagCompound) {
        ...
        transitEdgesList.forEach { transitEdgeTag ->
            ...
            typeList.forEach { transitEdgeEntry ->
                this.chunkTransitEdges[blockPos]!!.add(
                    TransitChunkEdge(blockPos)
        ...
        crossSectionList.forEach { crossSectionTag ->
            val blockPos =
                NBTUtil.getPosFromTag((crossSectionTag as NBTTagCompound).getCompoundTag(NBT_KEY_CROSS_SECTION_POS))
        ...
        crossSectionTransitList.forEach { transitEdgeTag ->
            val blockPos =
                NBTUtil.getPosFromTag(
                    (transitEdgeTag as NBTTagCompound).getCompoundTag(
                        NBT_KEY_CROSS_SECTION_TRANSIT_POS
                    )
                )
            val typeList =
                transitEdgeTag.getTagList(NBT_KEY_CROSS_SECTION_TRANSIT_EDGES_LIST, Constants.NBT.TAG_COMPOUND)

            this.crossSectionTransitEdges[blockPos] = mutableSetOf()

            typeList.forEach { transitEdgeEntry ->
                this.crossSectionTransitEdges[blockPos]!!.add(
                    TransitCrossSectionEdge(blockPos)
                        .apply { deserializeNBT(transitEdgeEntry as NBTTagCompound) })
            }
        }
    }
